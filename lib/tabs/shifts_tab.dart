...
class ShiftsTabState extends RefreshableState<ShiftsTab> {
  ...
  Widget getTitle() {
    return Text(
        '${_stl == ShiftsToLoad.today ? "today's" : "Relevant"} shifts');
  }
  ...
  Future<void> mainRefresh() async {
    ...
    final DateTime startDate =
        now.subtract(const Duration(days: daysEitherSide));
    ...
    List<Shift> allDayShifts = <Shift>[],
        previousShifts = <Shift>[],
        nextShifts = <Shift>[];
    for (final dynamic shiftData in shiftsData) {
      DateTime shiftStart =
          DateTime.tryParse(shiftData['start_datetime'] as String);
      final int duration = shiftData['duration'] as int;
      if (shiftStart == null || duration == null) {
        continue;
      }
      shiftStart = DateTime(shiftStart.year, shiftStart.month, shiftStart.day,
          shiftStart.hour, shiftStart.minute, shiftStart.second);
    ...
    for (final Shift shift in previousShifts) {
      if (latestPreviousShift == null ||
          shift.start.isAfter(latestPreviousShift)) {
    ...
    for (final Shift shift in nextShifts) {
      if (earliestNextShift == null ||
          shift.start.isBefore(earliestNextShift)) {
        earliestNextShift = shift.start;
      }
    }
    nextShifts = nextShifts
        .where((Shift entry) => entry.start.isAtSameMomentAs(earliestNextShift))
        .toList();
    allDayShifts = allDayShifts
        .where((Shift entry) =>
            entry.start.year == now.year &&
            entry.start.month == now.month &&
            entry.start.day == now.day)
        .toList();
    for (final List<Shift> shiftsList in <List<Shift>>[
      ...
      shiftsList.sort((Shift a, Shift b) =>
          a.volunteers.length.compareTo(b.volunteers.length));
